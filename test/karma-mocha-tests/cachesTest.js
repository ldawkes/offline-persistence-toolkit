define([
	...
	describe("cachestest", function () {
		...
		it("cache match test for 1.4.1 and below", function () {
			...
			var testCases = [
				...
				},
				{
					...
					matchResult: "response body for updating employee with id 209.",
					matchAllResult: [
						"response body for updating employee with id 209.",
				...
				},
				{
					...
					matchAllResult: [
						"response body for all departments uncompressed",
		...
		it("basic cache manager test", function () {
			var testCache;
			return new Promise(function (resolve, reject) {
				offlineCacheManager
					.delete("testcachemanager")
					.then(function () {
			...
			return new Promise(function (resolve, reject) {
				...
				offlineCacheManager
					...
					.then(function (keysArray) {
						...
						var requests = [
							new Request("http://localhost:7001/testcacheaddall1"),
		...
		it("cache manager match test", function () {
			return new Promise(function (resolve, reject) {
				var testCache1;
				var testCache2;
				var requestToMatch;
				offlineCacheManager
					.delete("cacheManagerMatchTestCache1")
					.then(function () {
		...
		it("cache shredding test collection", function () {
			var data = [
				{
					DepartmentId: 1001,
			...
			return new Promise(function (resolve, reject) {
				var testCache;
				offlineCacheManager
					.delete("cacheShreddingTestCollection")
					.then(function () {
		...
		it("cache shredding test single", function () {
			var data = {
				DepartmentId: 1001,
				DepartmentName: "ADFPM 1001 neverending",
				LocationId: 200,
			...
			return new Promise(function (resolve, reject) {
				var testCache;
				offlineCacheManager
					.delete("cacheShreddingTestSingle")
					.then(function () {
			...
			return new Promise(function (resolve, reject) {
				var addRequestResponseSequentially = function (dataSet, index) {
					if (!dataSet || dataSet.length === index) {
						...
						cache
							.put(dataSet[index].request.clone(), dataSet[index].response.clone())
							.then(function () {
								waitForSeconds(1);
		...
		var checkMatchAllResult = function (responseArray, expectedTextArray) {
			if ((!responseArray || responseArray.length === 0) && (!expectedTextArray || expectedTextArray.length === 0)) {
				return Promise.resolve(true);
			} else if (
				responseArray &&
				expectedTextArray &&
