define([
	...
	persistenceManager.init().then(function () {
		describe("persistenceSyncManager", function () {
			...
			it("beforeSyncRequest event", function (done) {
				var validateHeader = null;
				mockFetch.addRequestReply(
					"*",
					"/testBeforeSyncRequest",
					{
						status: 200,
						body: "Ok",
					},
					...
					.then(function (registration) {
						...
						fetch("/testBeforeSyncRequest")
							.then(function (response) {
								...
								persistenceManager
									.getSyncManager()
									.addEventListener("beforeSyncRequest", beforeSyncRequestResolveNull, "/testBeforeSyncRequest");
							...
							.then(function () {
								...
								persistenceManager
									.getSyncManager()
									.removeEventListener("beforeSyncRequest", beforeSyncRequestResolveNull, "/testBeforeSyncRequest");
								persistenceManager.forceOffline(true);
								// beforeSyncRequest listener returns Promise resolves to action: continue
								persistenceManager
									.getSyncManager()
									.addEventListener("beforeSyncRequest", beforeSyncRequestResolveContinue, "/testBeforeSyncRequest");
							...
							.then(function () {
								...
								persistenceManager
									.getSyncManager()
									.removeEventListener("beforeSyncRequest", beforeSyncRequestResolveContinue, "/testBeforeSyncRequest");
								persistenceManager.forceOffline(true);
								// beforeSyncRequest listener returns Promise resolves to action: skip
								persistenceManager
									.getSyncManager()
									.addEventListener("beforeSyncRequest", beforeSyncRequestResolveSkip, "/testBeforeSyncRequest");
							...
							.then(function () {
								persistenceManager
									.getSyncManager()
									.removeEventListener("beforeSyncRequest", beforeSyncRequestResolveSkip, "/testBeforeSyncRequest");
								persistenceManager.forceOffline(true);
								// beforeSyncRequest listener returns Promise resolves to action: stop
								persistenceManager
									.getSyncManager()
									.addEventListener("beforeSyncRequest", beforeSyncRequestResolveStop, "/testBeforeSyncRequest");
								return fetch("/testBeforeSyncRequest");
							})
							.then(function () {
								assert.ok(true, "Received Response when offline");
							...
							.then(function () {
								persistenceManager
									.getSyncManager()
									.removeEventListener("beforeSyncRequest", beforeSyncRequestResolveStop, "/testBeforeSyncRequest");
								persistenceManager.forceOffline(true);
								// beforeSyncRequest listener returns Promise resolves to action: replay
								persistenceManager
									.getSyncManager()
									.addEventListener("beforeSyncRequest", beforeSyncRequestResolveReplay, "/testBeforeSyncRequest");
							...
							.then(function () {
								persistenceManager
									.getSyncManager()
									.removeEventListener("beforeSyncRequest", beforeSyncRequestResolveReplay, "/testBeforeSyncRequest");
								registration.unregister().then(function (unregistered) {
									assert.ok(unregistered == true, "unregistered scope");
									done();
								});
			...
			it("syncRequest event", function (done) {
				var validateHeader = null;
				mockFetch.addRequestReply(
					"*",
					"/testSyncRequest",
					...
					.then(function (registration) {
						...
						fetch("/testSyncRequest")
							.then(function (response) {
								...
								persistenceManager
									.getSyncManager()
									.addEventListener("syncRequest", syncRequestResolveNull, "/testSyncRequest");
								validateHeader = "X-Resolve-Null";
							...
							.then(function () {
								...
								persistenceManager
									.getSyncManager()
									.removeEventListener("syncRequest", syncRequestResolveNull, "/testSyncRequest");
								persistenceManager.forceOffline(true);
								// beforeSyncRequest listener returns Promise resolves to action: continue
								persistenceManager
									.getSyncManager()
									.addEventListener("syncRequest", syncRequestResolveContinue, "/testSyncRequest");
								validateHeader = "X-Resolve-Continue";
								var request = new Request("/testSyncRequest", {
									headers: new Headers({
										"X-Resolve-Continue": "true",
							...
							.then(function () {
								...
								persistenceManager
									.getSyncManager()
									.removeEventListener("syncRequest", syncRequestResolveContinue, "/testSyncRequest");
								persistenceManager.forceOffline(true);
								// beforeSyncRequest listener returns Promise resolves to action: stop
								persistenceManager
									.getSyncManager()
									.addEventListener("syncRequest", syncRequestResolveStop, "/testSyncRequest");
							...
							.then(function () {
								persistenceManager
									.getSyncManager()
									.removeEventListener("syncRequest", syncRequestResolveStop, "/testSyncRequest");
								registration.unregister().then(function (unregistered) {
									assert.ok(unregistered == true, "unregistered scope");
									done();
								});
			...
			it("insertRequest()", function (done) {
				persistenceManager
					.register({
			...
			it("removeRequest()", function (done) {
				persistenceManager
					.register({
			...
			it("updateRequest()", function (done) {
				persistenceManager
					.register({
					...
					.then(function (registration) {
						...
						persistenceManager
							...
							.then(function (syncLog) {
								...
								persistenceManager
									.getSyncManager()
									.updateRequest(requestId, syncLog[0].request)
									.then(function () {
										return persistenceManager.getSyncManager().getSyncLog();
									})
									.then(function (syncLog) {
			...
			it("timeout fetch()", function (done) {
				mockFetch.addRequestReply(
					"GET",
					"/testTimeout",
					{
				...
				mockFetch.addRequestReply("OPTIONS", "/testTimeout", new Promise(function (resolve, reject) {}), function () {
					assert.ok(true, "Mock Fetch received Request when online");
				});
			...
			it("timeout fetch() URL", function (done) {
				mockFetch.addRequestReply(
					"GET",
					"/testURLTimeout",
					{
						...
						fetch("/testURLTimeout")
							...
							.then(function () {
								...
								return persistenceManager
									.getSyncManager()
									.sync({ preflightOptionsRequest: "/testURLTimeout", preflightOptionsRequestTimeout: 1000 });
							})
							.then(
								function () {},
								function (err) {
				...
				persistenceManager
					...
					.then(function (registration) {
						...
						fetch("/testUndoRedo").then(function (response) {
							assert.ok(true, "Received Response when online");
							persistenceManager.forceOffline(true);
							var request = new Request("/testUndoRedo", {
								method: "PUT",
								body: JSON.stringify([{ ID: 2, name: "Test2" }]),
							});
							fetch(request)
								.then(function (response) {
									assert.ok(true, "Received Response when offline");
								...
								.then(function (syncLogItem) {
									return syncLogItem
										...
										.then(function (responseJSON) {
											assert.ok(
												responseJSON.body.text == JSON.stringify([{ ID: 2, name: "Test1" }]),
												"undo data correct"
											);
											return syncLogItem.redo();
										})
										...
										.then(function (responseJSON) {
											assert.ok(
												responseJSON.body.text == JSON.stringify([{ ID: 2, name: "Test2" }]),
												"redo data correct"
											);
										});
								})
