...
define(["require", "../persistenceUtils", "../persistenceStoreManager", "./defaultCacheHandler", "./logger"], function (
	require,
	persistenceUtils,
	...
	PersistenceSyncManager.prototype.addEventListener = function (type, listener, scope) {
		logger.log(
			"Offline Persistence Toolkit PersistenceSyncManager: addEventListener() for type: " +
				type +
				" and scope: " +
				scope
		);
	...
	PersistenceSyncManager.prototype.removeEventListener = function (type, listener, scope) {
		logger.log(
			"Offline Persistence Toolkit PersistenceSyncManager: removeEventListener() for type: " +
				type +
				" and scope: " +
				scope
		);
		this._eventListeners = this._eventListeners.filter(function (eventListener) {
			if (
				type.toLowerCase() == eventListener.type &&
	...
	PersistenceSyncManager.prototype.insertRequest = function (request, options) {
		logger.log(
			"Offline Persistence Toolkit PersistenceSyncManager: insertRequest() for Request with url: " + request.url
		);
		var localVars = {};
		return _getSyncLogStorage()
			.then(function (store) {
					...
					if (undoRedoDataArray != null) {
						return _getRedoUndoStorage().then(function (redoUndoStore) {
							var storeUndoRedoData = function (i) {
								if (i < undoRedoDataArray.length && undoRedoDataArray[i] != null) {
									return redoUndoStore
										.upsert(localVars.requestId, localVars.metadata, undoRedoDataArray[i])
										.then(function () {
	...
	PersistenceSyncManager.prototype.removeRequest = function (requestId) {
		logger.log(
			"Offline Persistence Toolkit PersistenceSyncManager: removeRequest() for Request with requestId: " + requestId
		);
		var self = this;
		var localVars = {};
		return _getSyncLogStorage()
	...
	PersistenceSyncManager.prototype._internalRemoveRequest = function (requestId, request, synclogStore) {
		...
		if (!synclogStore) {
			promises.push(
				_getSyncLogStorage().then(function (synclogStore) {
		...
		return Promise.all(promises)
			...
			.catch(function (error) {
				logger.log(
					"Offline Persistence Toolkit PersistenceSyncManager: removeRequest() error for Request with requestId: " +
						requestId
				);
			});
	};
	...
	PersistenceSyncManager.prototype.updateRequest = function (requestId, request) {
		logger.log(
			"Offline Persistence Toolkit PersistenceSyncManager: updateRequest() for Request with requestId: " + requestId
		);
		...
		var syncPromise = new Promise(function (resolve, reject) {
			self.getSyncLog().then(function (value) {
				...
				var replayRequestArray = function (requests) {
					...
					if (requests.length > 0) {
						logger.log(
							"Offline Persistence Toolkit PersistenceSyncManager: Processing sync, # of requests in sync log: " +
								requests.length
						);
						...
						_dispatchEvent(
							self,
							"beforeSyncRequest",
							{ requestId: requestId, request: requestClone.clone() },
							request.url
						).then(function (eventResult) {
							if (_checkStopSync(eventResult)) {
								logger.log(
									"Offline Persistence Toolkit PersistenceSyncManager: Sync stopped by beforeSyncRequest event listener"
								);
								resolve();
								return;
							}
							...
							if (eventResult.action !== "skip") {
								if (eventResult.action === "replay") {
									logger.log(
										"Offline Persistence Toolkit PersistenceSyncManager: Replay request from beforeSyncRequest event listener"
									);
									// replay the provided request instead of what's in the sync log
									request = eventResult.request;
								}
								requestClone = request.clone();
								_checkURL(self, request).then(
									function () {
										...
										logger.log(
											"Offline Persistence Toolkit PersistenceSyncManager: Replaying request with url: " + request.url
										);
										var fetchFunc;
										...
										fetchFunc(request).then(
											function (response) {
												...
												persistenceUtils._cloneResponse(response, { url: request.url }).then(function (responseClone) {
													logger.log(
														"Offline Persistence Toolkit PersistenceSyncManager: Dispatching syncRequest event"
													);
													_dispatchEvent(
														self,
														"syncRequest",
														{ requestId: requestId, request: requestClone.clone(), response: responseClone.clone() },
														request.url
													).then(function (dispatchEventResult) {
														if (!_checkStopSync(dispatchEventResult)) {
															logger.log(
																"Offline Persistence Toolkit PersistenceSyncManager: Removing replayed request"
															);
															self._internalRemoveRequest(requestId, request).then(
																function () {
																	requests.shift();
	...
	function _checkURL(persistenceSyncManager, request) {
		...
		if (
			...
			return new Promise(
				(function (preflightOptionsRequestId) {
					return function (resolve, reject) {
						...
						setTimeout(function () {
							if (!self._repliedOptionsRequest && self._preflightOptionsRequestId == preflightOptionsRequestId) {
								reject(false);
							}
						}, requestTimeout);
						self._browserFetch(preflightOptionsRequest).then(
							function (result) {
	...
	function _updateLocalStore(redoUndoDataArray, isUndo) {
		...
		var applyUndoRedoItem = function (i) {
			if (i < redoUndoDataArrayCount) {
				...
				if (operation == "upsert" || (operation == "remove" && isUndo)) {
					...
					return persistenceStoreManager.openStore(storeName).then(function (store) {
						if (dataArray.length == 1 && dataArray[0].value == null && dataArray[0].key != null) {
	...
	function _eventFilterFunction(eventType, url) {
		return function (eventListener) {
			return (
				eventType.toLowerCase() == eventListener.type &&
				((url != null && url.match(eventListener.scope)) || url == null || eventListener.scope == null)
			);
		};
	}
