define([
	...
	persistenceManager.init().then(function () {
		describe("persist/integration", function () {
			...
			it("Integration - sync replay with cache handling", function (done) {
				mockFetch.addRequestReply(
					"GET",
					"/testSyncCacheHandling/1001",
					{
						status: 200,
						body: JSON.stringify({
							DepartmentId: 1001,
			...
			it("Integration - server side deletion", function (done) {
				mockFetch.addRequestReply(
					"GET",
					"/testServerDeletion",
					{
						status: 200,
						body: JSON.stringify([
							{ DepartmentId: 1001, DepartmentName: "AA", LocationId: 200, ManagerId: 300 },
							{ DepartmentId: 556, DepartmentName: "BB", LocationId: 200, ManagerId: 300 },
			...
			it("Integration - server side deletion with query parameters", function (done) {
				mockFetch.addRequestReply(
					"GET",
					"/testParameterServerDeletion?offset=0&limit=25",
					{
						status: 200,
						body: JSON.stringify({
							hasMore: false,
				...
				persistenceManager
					...
					.then(function (registration) {
						...
						registration.addEventListener("fetch", defaultTestResponseProxy.getFetchEventListener());
						var store;
						fetch("/testParameterServerDeletion?offset=0&limit=25")
							.then(function (response) {
								assert.ok(true, "Received Response when online");
								return persistenceStoreManager.openStore("DepartmentsWQ");
							})
			...
			it("Integration - server side deletion with incomplete collection", function (done) {
				mockFetch.addRequestReply(
					"GET",
					"/testIncompleteServerDeletion?LocationId=200",
					{
						status: 200,
						body: JSON.stringify([
							{ DepartmentId: 1001, DepartmentName: "AA", LocationId: 200, ManagerId: 300 },
				...
				persistenceManager
					...
					.then(function (registration) {
						...
						registration.addEventListener("fetch", defaultTestResponseProxy.getFetchEventListener());
						var store;
						fetch("/testIncompleteServerDeletion?LocationId=200")
							.then(function (response) {
								assert.ok(true, "Received Response when online");
								return persistenceStoreManager.openStore("DepartmentsIC");
							})
			...
			it("Integration", function (done) {
				...
				mockFetch.addRequestReply(
					"GET",
					"/testIntegration?q=DepartmentId=1001",
					{
						status: 200,
						body: JSON.stringify([
							{ DepartmentId: 1001, DepartmentName: "ADFPM 1001 neverending", LocationId: 200, ManagerId: 300 },
				...
				persistenceManager
					...
					.then(function (registration) {
						...
						fetch("/testIntegration")
							...
							.then(function (payload) {
								var data = JSON.parse(payload);
								assert.ok(data.length === 1 && data[0].DepartmentName === "ADFPM 1001 neverending");
			...
			it("Conflict Resolution", function () {
				mockFetch.addRequestReply(
					"GET",
					"/testConflict/1001",
					{
				...
				persistenceManager.register({ scope: "/testConflict" }).then(function (registration) {
