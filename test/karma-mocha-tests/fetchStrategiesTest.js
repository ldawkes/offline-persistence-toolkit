define([
	...
	persistenceManager.init().then(function () {
		describe("fetchStrategies", function () {
			...
			it("getCacheFirstStrategy()", function (done) {
				...
				persistenceManager
					...
					.then(function (registration) {
						var defaultTestResponseProxy = defaultResponseProxy.getResponseProxy({
							fetchStrategy: fetchStrategies.getCacheFirstStrategy({ serverResponseCallback: serverCallback }),
						});
						registration.addEventListener("fetch", defaultTestResponseProxy.getFetchEventListener());

						fetch("/testCacheFirst")
			...
			it("getCacheFirstStrategy() no serverCallback specified", function (done) {
				mockFetch.addRequestReply(
					"GET",
					"/testCacheFirstNoServerCallback",
					{
						status: 200,
						body: JSON.stringify([
							{ DepartmentId: 1001, DepartmentName: "ADFPM 1001 neverending", LocationId: 200, ManagerId: 300 },
			...
			it("getCacheFirstStrategy() backgroundFetch disabled", function (done) {
				mockFetch.addRequestReply(
					"GET",
					"/testCacheFirstBackgroundFetchDisabled",
					{
						status: 200,
						body: JSON.stringify([
							{ DepartmentId: 1001, DepartmentName: "ADFPM 1001 neverending", LocationId: 200, ManagerId: 300 },
				...
				persistenceManager
					...
					.then(function (registration) {
						var defaultTestResponseProxy = defaultResponseProxy.getResponseProxy({
							fetchStrategy: fetchStrategies.getCacheFirstStrategy({ backgroundFetch: "disabled" }),
						});
						registration.addEventListener("fetch", defaultTestResponseProxy.getFetchEventListener());
			...
			it("getCacheIfOffline()", function (done) {
				mockFetch.addRequestReply(
					"GET",
					"/testCacheIfOffline",
					{
			...
			it("getCacheIfOffline() HEAD", function (done) {
			...
			it("getCacheIfOffline() response NotOk", function (done) {
