...
define(["./defaultCacheHandler", "../persistenceStoreManager", "../persistenceUtils", "./logger"], function (
	cacheHandler,
	...
	OfflineCache.prototype.match = function (request, options) {
		...
		return self
			._getCacheEntries(request, options)
	...
	OfflineCache.prototype.matchAll = function (request, options) {
		...
		return this._getCacheEntries(request, options)
			.then(function (cacheEntries) {
				var ignoreVary = options && options.ignoreVary;
	...
	OfflineCache.prototype._getStore = function () {
		...
		if (self._createStorePromise) {
			...
			var localStore;
			self._createStorePromise = persistenceStoreManager
				.openStore(self._storeName, {
	...
	OfflineCache.prototype._getShreddedNamesStore = function () {
		...
		if (self._createShreddedNamesStorePromise) {
			...
			self._createShreddedNamesStorePromise = persistenceStoreManager
				.openStore(self._shreddedNamesStoreName, {
					index: ["metadata.baseUrl", "metadata.url", "metadata.created"],
					skipMetadata: true,
				})
				.then(function (store) {
	...
	function _applyVaryForAllMatches(ignoreVary, request, cacheEntries) {
		...
		if (cacheEntries && cacheEntries.length) {
			...
			responseDataArray = filteredArray.map(function (entry) {
				return entry.value.responseData;
			});
		}
	...
	function _applyVaryCheck(ignoreVary, request, cacheValue) {
		...
		if (!varyValue) {
			...
			for (var index = 0; index < varyArray.length; index++) {
				...
				logger.log(
					"Offline Persistence Toolkit OfflineCache: Request HTTP Vary header value: " + requestVaryHeaderValue
				);
				logger.log(
					"Offline Persistence Toolkit OfflineCache: Cached HTTP Vary header value: " + cachedRequestVaryHeaderValue
				);
				if (
					(!cachedRequestVaryHeaderValue && !requestVaryHeaderValue) ||
					(cachedRequestVaryHeaderValue &&
						requestVaryHeaderValue &&
						cachedRequestVaryHeaderValue === requestVaryHeaderValue)
				) {
				} else {
					return false;
	...
	OfflineCache.prototype.put = function (request, response) {
		...
		return self
			...
			.then(function (requestResponsePair) {
				...
				if (!hasShredder) {
					return cacheStore.upsert(requestResponsePair.key, requestResponsePair.metadata, requestResponsePair.value);
				} else {
					return cacheHandler.shredResponse(request, response).then(function (shreddedPayload) {
						...
						storePromises.push(
							cacheStore.upsert(requestResponsePair.key, requestResponsePair.metadata, requestResponsePair.value)
						);
						storePromises.push(cacheHandler.cacheShreddedData(shreddedPayload, requestResponsePair.metadata));

						return self
							._updateShreddedStoreNames(
								shreddedPayload.map(function (entry) {
									return entry.name;
								})
							)
							.then(function () {
	...
	function _buildBodyAbstract(shreddedPayload) {
		var bodyAbstract = shreddedPayload.map(function (element) {
			return {
				name: element.name,
				keys: element.keys
					? element.keys.reduce(function (processedKeys, key) {
							if (key) {
	...
	OfflineCache.prototype.delete = function (request, options) {
		if (request) {
			...
			logger.warn(
				"Offline Persistence Toolkit OfflineCache: delete() request is a required parameter. To clear the cache, please call clear()"
			);
		...
		return self
			...
			.then(function (cacheStore) {
				if (cacheHandler.hasShredder(request)) {
					...
					return cacheStore.find(searchCriteria).then(function (dataArray) {
						if (dataArray && dataArray.length) {
							...
							return Promise.all(promises).then(function () {
								logger.log(
									"Offline Persistence Toolkit OfflineCache: all matching entries are deleted from both the cache store and the shredded store."
								);
								return true;
							});
						} else {
					...
					return _getStoreKeys(cacheStore, request, options)
			...
			.catch(function (error) {
				logger.log(
					"Offline Persistence Toolkit OfflineCache: error occurred delete() for Request with url: " + request.url
				);
				return false;
			});
	};
			...
			.then(function (keysArray) {
				...
				keysArray.forEach(function (key) {
					requestsPromiseArray.push(
						self._store.findByKey(key).then(function (entry) {
