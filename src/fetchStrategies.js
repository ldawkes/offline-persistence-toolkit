...
define(["./persistenceManager", "./persistenceUtils", "./impl/defaultCacheHandler", "./impl/logger"], function (
	persistenceManager,
	persistenceUtils,
	cacheHandler,
	logger
			...
			if (serverResponseCallback) {
				var wrappedServerResponseCallback = function (request, response) {
					...
					return persistenceUtils
						._cloneResponse(response, { url: request.url })
						.then(function (responseClone) {
	...
	function getCacheIfOfflineStrategy() {
		return function (request, options) {
			...
			if (persistenceManager.isOnline()) {
				return persistenceManager.browserFetch(request).then(
					function (response) {
				...
				if (!queryResponse) {
					...
					_checkCacheForMatch(request).then(function (cachedResponse) {
						if (cachedResponse) {
							...
							persistenceManager.browserFetch(request).then(
								function (response) {
	...
	return { getCacheFirstStrategy: getCacheFirstStrategy, getCacheIfOfflineStrategy: getCacheIfOfflineStrategy };
});
