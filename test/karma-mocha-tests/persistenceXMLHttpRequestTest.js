	...
	persistenceManager.init().then(function () {
		describe("PersistenceXMLHttpRequest", function () {
					...
					.then(function (registration) {
						...
						xhr.onreadystatechange = function (event) {
							...
							if (this.readyState == 1) {
								...
								assert.ok(
									this.getAllResponseHeaders().indexOf("content-type: test-mime") >= 0,
									"responseHeader is correct"
								);
							}
						};
			...
			it("test binary data handling without URL support", function (done) {
				generateMultipartFormWithBlob({ firstName: "Bob", lastName: "Smith" }, true).then(function (
					multipartDataToTest
				) {
					var headers = new Headers({ "Content-Type": "image/png" });
