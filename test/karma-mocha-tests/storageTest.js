define([
	...
	describe("storage test", function () {
		...
		describe("basic storage test", function () {
			it(":local", function () {
				var testParam = { name: "local", factory: localPersistenceStoreFactory };
				...
				return new Promise(function (resolve, reject) {
					persistenceStoreManager
						...
						.then(function (keysArray) {
							assert.ok(
								matchResultSetIgnoreOrder(keysArray, [
									"testBasicStorageKey1",
									"testBasicStorageKey2",
									"testBasicStorageKey3",
								]),
								"all rows are found."
							);
			...
			it(":pouchDB", function () {
				var testParam = { name: "pouchDB", factory: pouchDBPersistenceStoreFactory };
				console.log("testing " + testParam.name);
				var testStoreName = "http://" + testParam.name;
				var testStore;
				persistenceStoreManager.registerStoreFactory(testStoreName, testParam.factory);
			...
			it(":array", function () {
				var testParam = { name: "array", factory: arrayPersistenceStoreFactory };
				console.log("testing " + testParam.name);
				var testStoreName = "http://" + testParam.name;
				var testStore;
				persistenceStoreManager.registerStoreFactory(testStoreName, testParam.factory);
		...
		it("store manager test", function () {
			return new Promise(function (resolve, reject) {
				try {
			...
			it(":local", function () {
				...
				var testCases = [
					...
					},
					{
						...
						findExpression: {
							selector: {
								$and: [{ "value.firstName": "first2" }, { "value.numberValue": { $ne: 30 } }],
							},
							fields: ["key"],
						},
						expectedResult: [
					...
					},
					{
						...
						findExpression: {
							selector: {
								$or: [{ "value.numberValue": { $gte: 30 } }, { "value.booleanValue": { $eq: true } }],
							},
							fields: ["key"],
						},
						expectedResult: [
		...
		describe("test row version detection", function () {
			it(":local", function () {
				var testParam = { name: "local", factory: localPersistenceStoreFactory };
				var testCases = [
					{
							...
							var executeTestCaseArray = function (testCases, index) {
								if (index === testCases.length) {
									...
									testStore
										.upsert(
											valueToInsert.key,
											valueToInsert.metadata,
											valueToInsert.value,
											valueToInsert.expectedVersionIdentifier
										)
										.then(function () {
										...
										.catch(function (upsertErr) {
											if (testCases[index].expectingConflict && upsertErr && upsertErr.status === 409) {
												assert.ok(true);
												return Promise.resolve();
											} else {
												assert.ok(false);
		...
		describe("test store version detection", function () {
			it(":local", function () {
				var testParam = { name: "local", factory: localPersistenceStoreFactory };
				...
				return new Promise(function (resolve, reject) {
					persistenceStoreManager
						...
						.then(function (store3) {
							assert.ok(store3);
							return testStoreDV.upsert(testStoreName + "key", { created: 908935353 }, { name: "name-dv" });
						})
						.then(function () {
							assert.ok(true, "row is inserted to dv store.");
							return testStoreV1.upsert(testStoreName + "key", { created: 908935353 }, { name: "name-v1" });
						})
						.then(function () {
							assert.ok(true, "row is inserted to v1 store");
							return testStoreV2.upsert(testStoreName + "key", { created: 908935353 }, { name: "name-v2" });
						})
						.then(function () {
		...
		describe("test store index", function () {
			it(":local", function () {
				var testParam = { name: "local", factory: localPersistenceStoreFactory };
		...
		it("test binary data handling", function () {
			...
			persistenceManager.init().then(function () {
				var blobFromString = new Blob([JSON.stringify("some random string", null, 2)], { type: "application/json" });
				...
				return new Promise(function (resolve, reject) {
					persistenceStoreManager
						...
						.then(function () {
							...
							return testStore.upsert("testBinaryKey1", { created: 53535251153 }, { logo: blobFromString });
						})
						.then(function () {
							assert.ok(true, "first row is inserted.");
						...
						.then(function (valueFound) {
							if (!valueFound || !valueFound.logo || blobFromStringSize !== valueFound.logo.size) {
						...
						.then(function (valueFound) {
							...
							if (!valueFound || !valueFound.logo || blobFromStringSize !== valueFound.logo.size) {
								...
								assert.ok(true, "first row is found.");
								return testStore.upsert("testBinaryKey2", { created: 98943434 }, { logo: blobFromArray });
							}
						})
						.then(function () {
							assert.ok(true, "second row is inserted.");
						...
						.then(function (valueFound1) {
							if (!valueFound1 || !valueFound1.logo || blobFromArraySize !== valueFound1.logo.size) {
						...
						.then(function (keysValue) {
							assert.ok(
								matchResultSetIgnoreOrder(keysValue, ["testBinaryKey1", "testBinaryKey2"]),
								"all rows are found."
							);
							resolve();
						})
		...
		it("test SQLite Adapter", function () {
			...
			return persistenceStoreManager
				.openStore(testStoreName, { version: "1.0" })
				.then(function (store) {
					assert.ok(true, "store opened");
					testStore = store;
					return store.upsert(testStoreName + "key", { created: 908935353 }, { name: "name-adapter" });
				})
				.then(function () {
		...
		it("test abstract store", function () {
