define([
	...
	persistenceManager.init().then(function () {
		describe("queryHandlers", function () {
			...
			it("getSimpleQueryHandler", function (done) {
						...
						body: JSON.stringify([
							{
								...
								establishedDate: "1999-01-01T08:30:40Z",
								LocationId: 200,
								ManagerId: 300,
							},
							{
								DepartmentId: 556,
								DepartmentName: "BB",
								establishedDate: "2010-01-01T08:30:40Z",
								LocationId: 200,
								ManagerId: 300,
							},
							{
								...
								establishedDate: "2005-01-01T08:30:40Z",
								LocationId: 200,
								ManagerId: 300,
								lastModified: "2005-01-01T08:30:40Z",
							},
						]),
					},
					function () {
				...
				persistenceManager
					...
					.then(function (registration) {
						var options = {
							queryHandler: queryHandlers.getSimpleQueryHandler("departments", null, dataMapping),
							jsonProcessor: {
								shredder: simpleJsonShredding.getShredder("departments", "DepartmentId", dataMapping),
								unshredder: simpleJsonShredding.getUnshredder(dataMapping),
							},
						};
							...
							fetch("/testSimpleQuery").then(function (response) {
								...
								fetch("/testSimpleQuery?DepartmentName=BB").then(function (response) {
									...
									response.json().then(function (responseData) {
										assert.ok(
											responseData.length == 1 && responseData[0].DepartmentName == "BB",
											"Returned the correct department"
										);
			...
			it("getOracleRestQueryHandler", function (done) {
				mockFetch.addRequestReply(
					"GET",
					"/testOracleRestQuery/556",
					{
						status: 200,
						body: JSON.stringify({
							DepartmentId: 556,
				...
				persistenceManager
					...
					.then(function (registration) {
						var options = {
							queryHandler: queryHandlers.getOracleRestQueryHandler("departments", null, dataMapping),
							jsonProcessor: {
								shredder: oracleRestJsonShredding.getShredder("departments", "DepartmentId", dataMapping),
								unshredder: oracleRestJsonShredding.getUnshredder(dataMapping),
							},
						};
						var defaultTestResponseProxy = defaultResponseProxy.getResponseProxy(options);
						registration.addEventListener("fetch", defaultTestResponseProxy.getFetchEventListener());
						...
						persistenceManager.forceOffline(true);
						fetch("/testOracleRestQuery?q=DepartmentName=BB&offset=0&limit=10")
							...
							.then(function (responseData) {
								assert.ok(
									responseData.items.length == 1 && responseData.items[0].DepartmentName == "BB",
									"Returned the correct department"
								);
								return fetch("/testOracleRestQuery?q=DepartmentName='BB'&offset=0&limit=10");
							})
							...
							.then(function (responseData) {
								assert.ok(
									responseData.items.length == 1 && responseData.items[0].DepartmentName == "ADFPM 1001 neverending",
									"Returned the correct department"
								);
								return fetch("/testOracleRestQuery?q=DepartmentId<556&offset=0&limit=10");
							})
							...
							.then(function (responseData) {
								assert.ok(
									responseData.items.length == 1 && responseData.items[0].DepartmentName == "Administration",
									"Returned the correct department"
								);
								return fetch("/testOracleRestQuery?q=DepartmentId>=556&offset=0&limit=10");
							})
							...
							.then(function (responseData) {
								...
								assert.ok(
									responseData.items[0].DepartmentName == "ADFPM 1001 neverending" ||
										responseData.items[1].DepartmentName == "ADFPM 1001 neverending",
									"Returned the correct department"
								);
								assert.ok(
									responseData.items[1].DepartmentName == "BB" || responseData.items[0].DepartmentName == "BB",
									"Returned the correct department"
								);
							...
							.then(function (responseData) {
								assert.ok(responseData.items.length == 2, "Returned the correct departments");
								assert.ok(
									responseData.items[0].DepartmentName == "Administration" ||
										responseData.items[1].DepartmentName == "Administration",
									"Returned the correct department"
								);
								assert.ok(
									responseData.items[1].DepartmentName == "BB" || responseData.items[0].DepartmentName == "BB",
							...
							.then(function (responseData) {
								...
								return fetch(
									"/testOracleRestQuery?q=DepartmentId=556%20AND%20DepartmentName=Administration&offset=0&limit=10"
								);
							})
							.then(function (response) {
								assert.ok(true, "Received Response when offline");
							...
							.then(function (responseData) {
								...
								return fetch(
									"/testOracleRestQuery?q=DepartmentName%20LIKE%20%27aDmInIsTrAtIoN%25%27&offset=0&limit=10"
								);
							})
							.then(function (response) {
								assert.ok(true, "Received Response when offline");
							...
							.then(function (responseData) {
								assert.ok(
									responseData.items.length == 1 &&
										responseData.items[0].DepartmentName == responseArray[1].DepartmentName,
									"Returned the correct department"
								);
							...
							.then(function (responseData) {
								assert.ok(
									responseData.items.length == 1 && responseData.items[0].DepartmentId === 1001,
									"1 document found using $in"
								);
							...
							.then(function (responseData) {
								...
								assert.ok(
									responseData.items[0].DepartmentId === 1001 || responseData.items[0].DepartmentId === 10,
									"DepartmentId is Correct"
								);
								assert.ok(
									responseData.items[1].DepartmentId === 1001 || responseData.items[1].DepartmentId === 10,
									"DepartmentId is Correct"
								);
								assert.ok(
									responseData.items[0].DepartmentId != responseData.items[1].DepartmentId,
							...
							.then(function (responseData) {
								...
								assert.ok(
									responseData.items[0].DepartmentName === "BB" ||
										responseData.items[0].DepartmentName === "ADFPM 1001 neverending",
									"DepartmentName is Correct"
								);
								assert.ok(
									responseData.items[1].DepartmentName === "BB" ||
										responseData.items[1].DepartmentName === "ADFPM 1001 neverending",
									"DepartmentName is Correct"
								);
								assert.ok(
									responseData.items[0].DepartmentName != responseData.items[1].DepartmentName,
									"Not the same Department"
								);
							...
							.then(function (responseData) {
								...
								assert.ok(
									responseData.items[0].LocationId === 200 &&
										responseData.items[1].LocationId === 200 &&
										responseData.items[2].LocationId === 200,
									"LocationId is Correct"
								);
								return fetch("/testOracleRestQuery?q=DepartmentId<>556&offset=0&limit=10");
							})
							.then(function (response) {
							...
							.then(function (responseData) {
								assert.ok(responseData.items.length == 2, "Returned the correct departments");
								assert.ok(
									responseData.items[0].DepartmentName == "BB" || responseData.items[1].DepartmentName == "BB",
									"Returned the correct department"
								);
								assert.ok(
									responseData.items[1].DepartmentName == "ADFPM 1001 neverending" ||
								...
								return fetch("/testOracleRestQuery?q=LocationId=200&orderBy=DepartmentName:desc");
							})
							.then(function (response) {
								assert.ok(true, "Received Response when offline");
								return response.json();
							...
							.then(function (responseData) {
								...
								assert.ok(
									arrayUtil.compareArray(departmentNameArray.slice().sort().reverse(), departmentNameArray),
							...
							.then(function (responseData) {
								...
								assert.ok(responseData.items.length == 3, "Returned the correct departments - sorting");
								assert.ok(
									arrayUtil.compareArray(departmentNameArray.slice().sort(), departmentNameArray),
							...
							.then(function (responseData) {
								...
								assert.ok(responseData.items.length == 2, "Returned the correct departments - sorting");
								assert.ok(
									arrayUtil.compareArray(departmentNameArray.slice().sort().reverse(), departmentNameArray),
