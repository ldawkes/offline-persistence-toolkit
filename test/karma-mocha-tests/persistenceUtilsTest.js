define(["persist/persistenceUtils", "formData", "persist/impl/logger", "MockFetch"], function (
	persistenceUtils,
	formData,
	logger,
	MockFetch
	...
	describe("persistenceUtilsTest", function () {
		...
		it("multipart form test", function () {
			return new Promise(function (resolve, reject) {
				...
				var parsedResult = persistenceUtils.parseMultipartFormData(
					multipartDataToTest.content,
					multipartDataToTest.contentType
				);
		...
		it("request to JSON with multipart data, no FormData support", function () {
			return new Promise(function (resolve, reject) {
				generateMultipartFormWithBlob({ firstName: "Bob", lastName: "Smith" }).then(function (multipartDataToTest) {
					var request = new Request("http://test", {
						method: "POST",
