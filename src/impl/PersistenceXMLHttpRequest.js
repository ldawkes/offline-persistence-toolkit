...
define(["../persistenceUtils", "./logger"], function (persistenceUtils, logger) {
   ...
   */
	PersistenceXMLHttpRequest.prototype.open = function (method, url, async, username, password) {
		...
		if (typeof async == "boolean" && !async) {
			throw new Error(
				"InvalidAccessError: Failed to execute 'open' on 'XMLHttpRequest': Synchronous requests are disabled on the XHR Adapter"
			);
		}
		this._method = method;
		this._url = url;
		...
		if (isFile) {
			...
			this._passthroughXHR.onreadystatechange = function () {
				...
				if (readyState == PersistenceXMLHttpRequest.DONE) {
					...
					if (self._responseType == null || self._responseType == "" || self._responseType == "text") {
						self._responseText = self._passthroughXHR.responseText;
					}
					self._responseURL = self._passthroughXHR.responseURL;
					if (self._responseType == null || self._responseType == "" || self._responseType == "document") {
   ...
   */
	PersistenceXMLHttpRequest.prototype.setRequestHeader = function (header, value) {
		logger.log(
			"Offline Persistence Toolkit PersistenceXMLHttpRequest: setRequestHeader() with header: " +
				header +
				" ,value: " +
				value
		);
		...
		this._requestHeaders[header] = oldValue ? (oldValue += "," + value) : value;
	};
		...
		if (this._passthroughXHR) {
			...
			try {
				...
				if (self.timeout && typeof self.timeout === "number") {
					_fetchWithTimeout(request, options, self)
   ...
   */
	...
	PersistenceXMLHttpRequest.prototype.addEventListener = function (eventType, listener) {
		logger.log(
			"Offline Persistence Toolkit PersistenceXMLHttpRequest: addEventListener() for event type: " + eventType
		);
	...
	PersistenceXMLHttpRequest.prototype.removeEventListener = function (eventType, listener) {
		logger.log(
			"Offline Persistence Toolkit PersistenceXMLHttpRequest: removeEventListener() for event type: " + eventType
		);
	...
	PersistenceXMLHttpRequest.prototype.dispatchEvent = function (event) {
		...
		switch (type) {
			...
			case "loadstart":
				if (this._isUpload) {
					this.upload._dispatchEvent(
						new PersistenceXMLHttpRequestProgressEvent("loadstart", false, false, self.upload)
					);
				}
				if (this.onloadstart) {
	...
	function _readyStateChange(self, state) {
		...
		if (self._isTimedOut == true && self._readyState == PersistenceXMLHttpRequest.DONE) {
	...
	function _isFileRequest(url) {
		...
		if (URL && URL.prototype) {
			...
			if (absoluteUrlOrigin != null && absoluteUrlOrigin != "null" && absoluteUrlOrigin.length > 0) {
	...
	function _getRequestInit(self, data) {
		...
		if (self._method !== "GET" && self._method !== "HEAD" && self._method !== "DELETE") {
		...
		if (headers.entries) {
			...
			do {
				...
				if (headerEntry["value"]) {
					...
					if (self._forceMimeType && headerName.toLowerCase() == "content-type") {
	...
	PersistenceXMLHttpRequestUpload.prototype.addEventListener = function (eventType, listener) {
		logger.log(
			"Offline Persistence Toolkit PersistenceXMLHttpRequestUpload: addEventListener() for event type: " + eventType
		);
		this._eventListeners[eventType] = this._eventListeners[eventType] || [];
		this._eventListeners[eventType].push(listener);
	};
	...
	PersistenceXMLHttpRequestUpload.prototype.removeEventListener = function (eventType, listener) {
		logger.log(
			"Offline Persistence Toolkit PersistenceXMLHttpRequestUpload: removeEventListener() for event type: " + eventType
		);
		var listeners = this._eventListeners[eventType] || [];
		var i;
		var listenersCount = listeners.length;
	...
	PersistenceXMLHttpRequestUpload.prototype._dispatchEvent = function (event) {
		logger.log(
			"Offline Persistence Toolkit PersistenceXMLHttpRequestUpload: dispatchEvent() for event type: " + event.type
		);
		var self = this;
		var type = event.type;
		var listeners = this._eventListeners[type] || [];
