define(["persist/oracleRestJsonShredding", "persist/impl/logger"], function (oracleRestJsonShredding, logger) {
	"use strict";
	...
	describe("oracleRestJsonShredding", function () {
		it("getShredder()", function (done) {
			...
			var singlePayloadJson = JSON.stringify({
				DepartmentId: 1001,
			...
			shredder(response).then(function (shreddedData) {
				...
				shredder(singleResponse).then(function (shreddedData) {
					...
					var shreddedDataJsonString = JSON.stringify(shreddedData[0].data);
					assert.ok(
						JSON.stringify([JSON.parse(singlePayloadJson)]) == shreddedDataJsonString,
						"shreddedData item data is correct"
					);
					assert.ok(shreddedData[0].keys[0] == 1001, "shreddedData item keys[0] is correct");
					done();
				});
		...
		it("getShredder() with complex key", function (done) {
			...
			shredder(response).then(function (shreddedData) {
				...
				assert.ok(JSON.stringify(data) == shreddedDataJsonString, "shreddedData item data is correct");
				assert.ok(
					JSON.stringify(shreddedData[0].keys[0]) == JSON.stringify([1001, 200]),
					"shreddedData item keys[0] is correct"
				);
				assert.ok(
					JSON.stringify(shreddedData[0].keys[1]) == JSON.stringify([556, 200]),
					"shreddedData item keys[0] is correct"
				);
				assert.ok(
					JSON.stringify(shreddedData[0].keys[2]) == JSON.stringify([10, 200]),
					"shreddedData item keys[0] is correct"
				);
				shredder(singleResponse).then(function (shreddedData) {
					assert.ok(shreddedData.length == 1, "shreddedData contains one item");
		...
		it("getShredder() with complex key with null", function (done) {
			var data = [
				{ DepartmentId: 1001, DepartmentName: "ADFPM 1001 neverending", LocationId: null, ManagerId: 300 },
			...
			shredder(response).then(function (shreddedData) {
				...
				assert.ok(JSON.stringify(data) == shreddedDataJsonString, "shreddedData item data is correct");
				assert.ok(
					JSON.stringify(shreddedData[0].keys[0]) == JSON.stringify([1001, null]),
					"shreddedData item keys[0] is correct"
				);
				assert.ok(
					JSON.stringify(shreddedData[0].keys[1]) == JSON.stringify([556, null]),
					"shreddedData item keys[0] is correct"
				);
				assert.ok(
					JSON.stringify(shreddedData[0].keys[2]) == JSON.stringify([10, null]),
					"shreddedData item keys[0] is correct"
				);
				shredder(singleResponse).then(function (shreddedData) {
					assert.ok(shreddedData.length == 1, "shreddedData contains one item");
			...
			unshredder(
				...
				response.json().then(function (updatedPayload) {
					...
					unshredder(
						...
						response.json().then(function (updatedPayload) {
							assert.ok(
								JSON.stringify(singlePayload[0]) == JSON.stringify(updatedPayload),
								"unshredded payload is correct"
							);
							done();
						});
					});
		...
		it("getUnshredder() with dataMapping", function (done) {
			var payload = [
				{
					DepartmentId: 1001,
					DepartmentName: "ADFPM 1001 neverending",
					establishedDate: new Date("1999-01-01T08:30:40Z").getTime(),
					LocationId: 200,
					ManagerId: 300,
				},
				{
					DepartmentId: 556,
					DepartmentName: "BB",
					establishedDate: new Date("2010-01-01T08:30:40Z").getTime(),
					LocationId: 200,
					ManagerId: 300,
				},
				{
					DepartmentId: 10,
					DepartmentName: "Administration",
					establishedDate: new Date("2005-01-01T08:30:40Z").getTime(),
					LocationId: 200,
					ManagerId: 300,
				},
			];
			...
			unshredder(
				...
				response.json().then(function (updatedPayload) {
					assert.ok(
						JSON.stringify(expectedPayload) == JSON.stringify(updatedPayload.items),
						"unshredded payload is correct"
					);
					done();
				});
