define([
	...
	persistenceManager.init().then(function () {
		xdescribe("persist/persistenceManager- file system storage", function () {
			...
			it("blob response", function (done) {
					...
					generateBlob().then(function (blob) {
						...
						persistenceManager
							...
							.then(function (registration) {
								var options = {
									jsonProcessor: {
										shredder: simpleBinaryDataShredding.getShredder("blobResponse"),
										unshredder: simpleBinaryDataShredding.getUnshredder(),
									},
								};
								...
								fetch("/blobResponse1")
									...
									.then(function (keys) {
										assert.ok(
											(keys[0] == "file:///blobResponse1A" && keys[1] == "file:///blobResponse2") ||
