...
define([
	...
	function getOracleRestQueryHandler(storeName, createQueryExp, dataMapping) {
		...
		var queryHandler = function (request, options) {
			if (request.method == "GET" || request.method == "HEAD") {
				...
				var findQuery = persistenceUtils._mapFindQuery(
					createQueryExp(normalized.searchCriteria),
					dataMapping,
					sortQuery
				);
				...
				if (shredder != null && unshredder != null) {
	...
	function _processQuery(request, storeName, findQuery, shredder, unshredder, offset, limit) {
		return persistenceManager
			.getCache()
			._internalMatch(request, { ignoreSearch: true, ignoreBody: true })
			.then(function (cacheEntryMetadata) {
					...
					if (cacheEntryMetadata.resourceType === "unknown") {
						...
						return persistenceStoreManager
							...
							.then(function (results) {
								...
								return persistenceManager
									.getCache()
									._matchByKey(request, cacheEntryMetadata.key, { ignoreBody: true })
									.then(function (response) {
									...
									if (collectionUrl) {
										return persistenceUtils.requestToJSON(request).then(function (requestObj) {
											...
											return persistenceUtils.requestFromJSON(requestObj).then(function (collectionRequest) {
												return persistenceManager
													.getCache()
													.match(collectionRequest, { ignoreSearch: true, ignoreBody: true })
													.then(function (response) {
														if (response) {
		...
		if (value) {
			...
			for (i = 0; i < queryExpArray.length; i++) {
				selectorQueryItem = parser.parse(queryExpArray[i], function (operatorValue, operands) {
					...
					if (operatorValue != "AND" && operatorValue != "OR" && operatorValue != ",") {
	...
	function getSimpleQueryHandler(storeName, ignoreUrlParams, dataMapping) {
		...
		var queryHandler = function (request, options) {
			if (request.method == "GET" || request.method == "HEAD") {
				...
				var findQuery = persistenceUtils._mapFindQuery(
					_createQueryFromUrlParams(normalized.searchCriteria, ignoreUrlParams),
					dataMapping
				);
		...
		if (searchParams) {
			...
			do {
				...
				if (queryParamEntry["value"] != null) {
					...
					if (!ignoreUrlParams || ignoreUrlParams.indexOf(queryParamName) == -1) {
						// build our query
	...
	return { getSimpleQueryHandler: getSimpleQueryHandler, getOracleRestQueryHandler: getOracleRestQueryHandler };
});
