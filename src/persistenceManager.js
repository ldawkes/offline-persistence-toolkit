...
define([
	...
	function PersistenceManager() {
		...
		Object.defineProperty(this, "_persistenceSyncManager", {
			value: new PersistenceSyncManager(
				this.isOnline.bind(this),
				this.browserFetch.bind(this),
	...
	PersistenceManager.prototype.isOnline = function () {
		...
		if (navigator.network && navigator.network.connection && navigator.network.connection.type == Connection.NONE) {
			online = false;
			logger.log(
				"Offline Persistence Toolkit PersistenceManager: Cordova network info plugin is returning online value: " +
					online
			);
		}
	...
	PersistenceManager.prototype.browserFetch = function (request) {
		...
		if (_isBrowserContext()) {
			...
			return new Promise(function (resolve, reject) {
				logger.log(
					"Offline Persistence Toolkit PersistenceManager: Calling browser fetch function for Request with url: " +
						request.url
				);
				if (request._browserRequest) {
					requestObj = request._browserRequest;
				}
				self._browserFetchFunc.call(window, requestObj).then(
					function (response) {
						resolve(response);
					},
					function (error) {
	...
	function _addBrowserEventListeners(persistenceManager) {
		...
		if (_isBrowserContext() && !self._addedBrowserEventListeners) {
			...
			if (event.request.url.match(registration["scope"]) != null) {
				...
				for (j = 0; j < eventListenerCount; j++) {
					if (registration._eventListeners[j]["type"] == eventType) {
						if (eventType == "fetch") {
							if (respondWithPromise === null && event._setPromiseCallbacks instanceof Function) {
	...
	function _replaceBrowserApis(persistenceManager) {
		...
		if (
			...
			if (!_isBrowserContext() && !persistenceManager._browserFetchFunc) {
		...
		if (_isBrowserContext() && !persistenceManager._browserFetchFunc && !persistenceManager._browserXMLHttpRequest) {
			// browser context
	...
	PersistenceRegistration.prototype.addEventListener = function (type, listener) {
		this._eventListeners.push({ type: type.toLowerCase(), listener: listener });
	};
	...
	function persistenceRequest(persistenceManager) {
		function PersistenceRequest(input, init) {
			...
			if (input._input) {
				...
				if (input.headers && requestInit && requestInit.body && requestInit.body instanceof FormData) {
			...
			var requestDefineProperty = function (requestProperty) {
				...
				if (propDescriptors && (propDescriptors.writable || propDescriptors.set)) {
			...
			if (boundary != null && boundary.indexOf("boundary=") > -1 && boundary.indexOf("form-data") > -1) {
					...
					if (self._init && self._init.body) {
						if (!(self._init.body instanceof FormData)) {
							...
							return _formDataToString(self._init.body, self._boundary).then(function (formDataText) {
								var formDataBlob = new Blob([formDataText], { type: self.headers.get("Content-Type") });
								return formDataBlob;
							});
						}
					}
			...
			this.clone = function () {
				...
				var self = this;
				if (self.headers && self._init && self._init.body && self._init.body instanceof FormData) {
	...
	function serviceWorkerFetch(persistenceManager) {
		...
		return function (input, init) {
			...
			return new Promise(function (resolve, reject) {
				persistenceManager._browserFetchFunc.call(self, request).then(
					function (response) {
						resolve(response);
					},
					function (error) {
