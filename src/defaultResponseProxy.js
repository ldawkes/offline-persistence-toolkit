...
define([
	...
	DefaultResponseProxy.prototype.getFetchEventListener = function () {
		...
		return function (event) {
			event.respondWith(self.processRequest(event.request));
		};
	};
	...
	DefaultResponseProxy.prototype.processRequest = function (request) {
		...
		return new Promise(function (resolve, reject) {
			...
			logger.log(
				"Offline Persistence Toolkit DefaultResponseProxy: Calling requestHandler for request with enpointKey: " +
					endpointKey
			);
			requestHandler
				.call(self, request)
				.then(function (response) {
					if (persistenceUtils.isCachedResponse(response)) {
						logger.log(
							"Offline Persistence Toolkit DefaultResponseProxy: Response is cached for request with enpointKey: " +
								endpointKey
						);
					...
					if (response.ok) {
						logger.log(
							"Offline Persistence Toolkit DefaultResponseProxy: Response is ok for request with enpointKey: " +
								endpointKey
						);
						return _applyCacheStrategy(self, request, response);
					} else {
						logger.log(
							"Offline Persistence Toolkit DefaultResponseProxy: Response is not ok for request with enpointKey: " +
								endpointKey
						);
						return response;
					}
				...
				.then(function (response) {
					...
					if (response.ok) {
						logger.log(
							"Offline Persistence Toolkit DefaultResponseProxy: Response is ok after cacheStrategy for request with enpointKey: " +
								endpointKey
						);
						...
						logger.log(
							"Offline Persistence Toolkit DefaultResponseProxy: Response is not ok after cacheStrategy for request with enpointKey: " +
								endpointKey
						);
						return null;
					}
				...
				.then(function (undoRedoDataArray) {
					if (!localVars.isReplayRequest) {
						return _insertSyncManagerRequest(
							request,
							undoRedoDataArray,
							localVars.isCachedResponse && !persistenceManager.isOnline()
						);
					}
				})
				.then(function () {
				...
				.catch(function (err) {
					logger.log(
						"Offline Persistence Toolkit DefaultResponseProxy: Insert Response in syncManager after error for request with enpointKey: " +
							endpointKey
					);
					if (!localVars.isReplayRequest) {
						_insertSyncManagerRequest(requestClone, null, true).then(
							function () {
								cacheHandler.unregisterEndpointOptions(endpointKey);
								reject(err);
							},
	...
	function _handlePutRequest(defaultResponseProxy, request) {
		var self = defaultResponseProxy;
		if (persistenceManager.isOnline()) {
			return persistenceManager.browserFetch(request.clone()).then(
				function (response) {
	...
	function _handleOfflinePutRequest(defaultResponseProxy, request) {
		...
		return persistenceUtils.requestToJSON(request).then(function (requestData) {
			...
			if (ifMatch || ifNoneMatch) {
				logger.log(
					"Offline Persistence Toolkit DefaultResponseProxy: Generating ETag for offline Response for default PUT Handler"
				);
	...
	function _handleOfflineDeleteRequest(defaultResponseProxy, request) {
		...
		return persistenceUtils.requestToJSON(request).then(function (requestData) {
			...
			return persistenceUtils.responseFromJSON(requestData).then(function (response) {
				...
				if (self._options && self._options.jsonProcessor && self._options.jsonProcessor.shredder) {
	...
	function _getUndoRedoDataForShreddedDataItem(request, storename, shreddedDataItem) {
		...
		var undoRedoData = function (i, dataArray) {
			...
			if (i < dataArray.length && request.method !== "GET" && request.method !== "HEAD") {
				...
				return persistenceStoreManager.openStore(storename).then(function (store) {
					return store.findByKey(key).then(
						function (undoRow) {
