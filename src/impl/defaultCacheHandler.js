...
define(["../persistenceUtils", "../persistenceStoreManager", "./logger"], function (
	persistenceUtils,
	...
	DefaultCacheHandler.prototype._isCompleteCollection = function (requestMetadata, shreddedObjArray) {
		if (!requestMetadata || (requestMetadata.method !== "GET" && requestMetadata.method !== "HEAD")) {
			return false;
				...
				if (varyValue) {
					if (varyValue === "*") {
						...
						for (var index = 0; index < varyFields.length; index++) {
							...
							var varyValue =
								!requestHeaders || !requestHeaders.get(varyField) ? "undefined" : requestHeaders.get(varyField);
		...
		return allKeys.filter(function (key) {
			...
			if (parts.length === 1) {
				...
				if (!options || !options.ignoreVary) {
					...
					if (varyPairs === 2) {
						var varyValue =
							!requestHeaders || !requestHeaders.get(varyPairs[0]) ? "undefined" : requestHeaders.get(varyPairs[0]);
						...
						for (var index = 0; index < varyPairs.length - 1; index++) {
							...
							var varyValue =
								!requestHeaders || !requestHeaders.get(varyFieldValueToCheck)
									? "undefined"
									: requestHeaders.get(varyFieldValueToCheck);
				...
				if (!options || !options.ignoreVary) {
					...
					for (var index = 0; index < pairs.length - 1; index++) {
						...
						var varyValue =
							!requestHeaders || !requestHeaders.get(varyFieldValueToCheck[0])
								? "undefined"
								: requestHeaders.get(varyFieldValueToCheck[0]);
	...
	DefaultCacheHandler.prototype._getJsonProcessor = function (requestUrl) {
		...
		for (var index = 0; index < allKeys.length; index++) {
			...
			if (requestUrl === JSON.parse(key).url) {
				...
				if (option && option.jsonProcessor && option.jsonProcessor.shredder && option.jsonProcessor.unshredder) {
	...
	DefaultCacheHandler.prototype.fillResponseBodyWithShreddedData = function (request, bodyAbstract, response) {
		...
		if (request.url != null && request.url.length > 0 && response.headers.get("x-oracle-jscpt-response-url") == null) {
