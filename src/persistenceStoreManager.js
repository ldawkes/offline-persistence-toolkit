...
define(["./impl/logger", "./impl/PersistenceStoreMetadata", "./pouchDBPersistenceStoreFactory"], function (
	logger,
	PersistenceStoreMetadata,
	...
	PersistenceStoreManager.prototype.openStore = function (name, options) {
		...
		return factory.createPersistenceStore(storeName, options).then(function (store) {
			...
			if (options && options.skipMetadata) {
				...
				return self
					._updateStoreMetadata(storeName, version)
					.then(function () {
						return store;
					})
