define([
	...
	persistenceManager.init().then(function (done) {
		describe("cacheStrategies", function () {
			this.timeout(30000);
			...
			it("getHttpCacheHeaderStrategy: expires", function (done) {
				...
				persistenceManager
					...
					.then(function (response) {
						...
						assert.ok(
							response.headers.get("x-oracle-jscpt-cache-expiration-date") == "Wed, 21 Oct 2015 07:28:00 GMT",
							"x-oracle-jscpt-cache-expiration-date is correct"
						);
						done();
					});
			});

			it("getHttpCacheHeaderStrategy: max-age", function (done) {
				mockFetch.addRequestReply(
					"GET",
					"/testMaxAge",
					{
			...
			it("getHttpCacheHeaderStrategy: If-Match", function (done) {
				mockFetch.addRequestReply(
					"GET",
					"/testIfMatch",
					{
				...
				persistenceManager
					...
					.then(function (response) {
						...
						assert.ok(true, "Received Response when online");
						persistenceManager.forceOffline(true);
						var init = { method: "GET", headers: { "If-Match": "XYZ123" }, mode: "cors" };
						var request = new Request("/testIfMatch", init);
						return fetch("/testIfMatch", request);
					})
					.then(function (response) {
						assert.ok(persistenceUtils.isCachedResponse(response), "Cached response");
						var init = { method: "GET", headers: { "If-Match": "ABC123" }, mode: "cors" };
						var request = new Request("/testIfMatch", init);
						return fetch("/testIfMatch", request);
					})
					.then(function (response) {
						...
						var init = { method: "GET", headers: { "If-None-Match": "ABC123" }, mode: "cors" };
						var request = new Request("/testIfMatch", init);
						return fetch("/testIfMatch", request);
					})
					.then(function (response) {
						...
						var init = { method: "GET", headers: { "If-None-Match": "XYZ123" }, mode: "cors" };
						var request = new Request("/testIfMatch", init);
						return fetch("/testIfMatch", request);
					})
					.then(function (response) {
			...
			it("getHttpCacheHeaderStrategy: must-revalidate", function (done) {
				mockFetch.addRequestReply(
					"GET",
					"/testMustRevalidate",
					{
						...
						statusText: "OK",
						body: "REPLY",
						headers: { "Cache-control": "must-revalidate", Expires: "Wed, 21 Oct 2015 07:28:00 GMT" },
					},
					function (request, response) {
						assert.ok(true, "Mock Fetch received Request when online");
					}
			...
			it("getHttpCacheHeaderStrategy: no-cache", function (done) {
				mockFetch.addRequestReply(
					"GET",
					"/testNoCache",
					{
				...
				persistenceManager
					...
					.then(function (registration) {
						...
						registration.addEventListener(
							"fetch",
							defaultTestResponseProxy.getFetchEventListener({ fetchStrategy: fetchStrategies.getCacheFirstStrategy() })
						);
			...
			it("getHttpCacheHeaderStrategy: no-store", function (done) {
				mockFetch.addRequestReply(
					"GET",
					"/testNoStore",
					{
