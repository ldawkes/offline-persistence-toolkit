...
define(["./impl/logger"], function (logger) {
	...
	function _isTextPayload(headers, isAppJsonText) {
		...
		if (isAppJsonText) {
			if (contentType && (contentType.match(/.*text\/.*/) || contentType.match(/.*application\/.*json.*/))) {
				return true;
			}
		} else {
	...
	function _copyProperties(sourceObj, targetObj, ignoreProps) {
		for (var k in sourceObj) {
			if (typeof sourceObj[k] !== "function" && !_isPrivateProperty(k) && ignoreProps.indexOf(k) === -1) {
	...
	function _addDateHeaderIfNull(headersData) {
		...
		if (!date) {
			logger.log(
				"Offline Persistence Toolkit persistenceUtils: Setting HTTP date header since it's null or not exposed"
			);
	...
	function _copyPayload(source, targetObj) {
		...
		if (source instanceof Request && _isMultipartPayload(source.headers)) {
			return _copyMultipartPayload(source, targetObj);
		}

		if (source instanceof Request || _isTextPayload(source.headers, true)) {
		...
		if (!(source instanceof Request) && typeof source.arrayBuffer === "function") {
	...
	function _createHeadersFromJsonObj(data, skipContentType) {
		...
		Object.keys(data.headers).forEach(function (key) {
			if (key !== "content-type" || (!skipContentType && key === "content-type")) {
	...
	function buildEndpointKey(request) {
		...
		var endPointKeyObj = {
			...
			id: Math.random()
				.toString(36)
				.replace(/[^a-z]+/g, ""), // @RandomNumberOK - Only used to internally keep track of request URLs
		};
	...
	function _mapData(idArray, dataArray, dataMapping) {
		...
		if (dataMapping) {
			...
			for (i = 0; i < totalCount; i++) {
				...
				var item =
					dataArray != null ? { metadata: itemMetadata, data: dataArray[i] } : { metadata: itemMetadata, data: null };
	...
	function _mapFindQuery(findQuery, dataMapping, sortingInput) {
		if (findQuery && dataMapping) {
			...
			if (filterCriterion) {
				findQuery.selector = _transformFilterCriterionToFindQuerySelector(
					dataMapping.mapFilterCriterion(filterCriterion)
				);
			}
		}
