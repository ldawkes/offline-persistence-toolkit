...
define(["./persistenceManager", "./persistenceUtils", "./impl/logger"], function (
	persistenceManager,
	persistenceUtils,
	logger
) {
	...
	function _handleExpires(request, response) {
		...
		if (
			...
			logger.log(
				"Offline Persistence Toolkit cacheStrategies: Set x-oracle-jscpt-cache-expiration-date header based on HTTP Expires header"
			);
		}
	}
	...
	function _handleMaxAge(request, response) {
		...
		if (cacheControlMaxAge != null) {
			if (persistenceUtils.isCachedResponse(response)) {
				...
				logger.log(
					"Offline Persistence Toolkit cacheStrategies: Set x-oracle-jscpt-cache-expiration-date header based on HTTP max-age header"
				);
			}
		}
	}
			...
			if (!persistenceManager.isOnline()) {
				...
				if (ifMatch && etag.indexOf(ifMatch) < 0) {
					...
					return persistenceUtils.responseToJSON(response).then(function (responseData) {
						...
						logger.log(
							"Offline Persistence Toolkit cacheStrategies: Returning Response status 412 based on ETag and HTTP If-Match header"
						);
						return persistenceUtils.responseFromJSON(responseData);
					});
				} else if (ifNoneMatch && etag.indexOf(ifNoneMatch) >= 0) {
					...
					return persistenceUtils.responseToJSON(response).then(function (responseData) {
						...
						logger.log(
							"Offline Persistence Toolkit cacheStrategies: Returning Response status 412 based on ETag and HTTP If-None-Match header"
						);
						return persistenceUtils.responseFromJSON(responseData);
					});
				}
			...
			if (!persistenceManager.isOnline()) {
				...
				if (mustRevalidate) {
					return persistenceUtils.responseToJSON(response).then(function (responseData) {
						...
						logger.log(
							"Offline Persistence Toolkit cacheStrategies: Returning Response status 504 based HTTP revalidation"
						);
						return persistenceUtils.responseFromJSON(responseData);
					});
				} else {
				...
				return persistenceManager.browserFetch(request).then(function (serverResponse) {
					if (serverResponse.status == 304) {
						...
						return persistenceManager
							...
							.then(function () {
								logger.log(
									"Offline Persistence Toolkit cacheStrategies: Removing old entry based on HTTP revalidation"
								);
